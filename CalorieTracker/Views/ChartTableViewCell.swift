...
class ChartTableViewCell: UITableViewCell {
    ...
    lazy var lineChartView:LineChartView = {
        let labelCount = min(6, data?.count ?? 0)
        let chartView = LineChartView()
        chartView.backgroundColor = .white
        chartView.rightAxis.enabled = false
        let yAxis = chartView.leftAxis
        yAxis.labelFont = .boldSystemFont(ofSize: 12)
        yAxis.setLabelCount(min(6, labelCount), force: false)
        ...
        chartView.xAxis.setLabelCount(min(6, labelCount), force: false)
        chartView.xAxis.labelTextColor = .systemGray3
        chartView.xAxis.axisLineColor = .systemGray5
        chartView.xAxis.drawGridLinesEnabled = false
        chartView.xAxis.labelRotationAngle = -45
        chartView.animate(xAxisDuration: 0.5)
        let marker = PillMarker(color: .white, font: UIFont.boldSystemFont(ofSize: 14), textColor: UIColor.tinder)
    ...
    func setData(data:[ChartPoint]){
        ...
        set1.circleRadius = 3
        set1.setCircleColor(UIColor.tinder)
        set1.lineWidth = 3
        set1.setColor(.red)
        set1.fill = Fill(color: UIColor.tinder)
        set1.fillAlpha = 0.7
